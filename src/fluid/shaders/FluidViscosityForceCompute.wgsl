...
// Viscosity via the Laplacian estimator (Eq. 8, STAR report):
//
//   a_i = nu * 2 * sum_j (m_j/rho_j) * (v_i-v_j) * (x_ij . grad_i W_ij)
//                          / (x_ij . x_ij + 0.01 h^2)
//
// x_ij . grad_i W_ij simplifies to r * dW/dr, since
// grad_i W_ij = (dW/dr) * (x_ij / r). No r > 0 guard needed (unlike
// the pressure force): at r = 0 the numerator is already 0, and the
// +0.01h^2 term keeps the denominator nonzero.
